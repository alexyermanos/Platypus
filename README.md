# Platypus
R package for the analysis of single-cell immune repertoires. Platypus contains functions that can be used to analyze VDJ and GEX libraries, both individually and separately. 
...
## Download and unzip the processed data files from the two COVID-19 patients found at doi.org/10.5281/zenodo.4140161
dir_to_covid_gex_individual <- list()
dir_to_covid_gex_individual[[1]] <- c("~/Downloads/covid_data/Patient1_GEX/")
dir_to_covid_gex_individual[[2]] <- c("~/Downloads/covid_data/Patient2_GEX/") 
...
### The full-length amino acid and nucleotide information can be additionally extracted using a newer function from v1.3. VDJ_per_cell directly from cellranger alignment files and no longer requires MiXCR. A warning is that this new extraction contains the sequence leader peptide which is often not desired for cloning.
