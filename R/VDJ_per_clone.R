#' Analyzes and processes the repertoire sequening data from cellranger vdj. This provides information on the single-cell level for each clone, as opposed to the output from VDJ_analyze.
...
#' @param reference.list Contains the reference sequence information in the same format as concat_ref.fasta file from the output of cellranger.2
...
#' @examples
#' \dontrun{
#' check_VDJperclone <- VDJ_per_clone(clonotype.list = output.from.VDJ_analyze,VDJ.out.directory = "path/to/cellranger/outs/")
#' }
VDJ_per_clone <- function(clonotype.list,
  ...
  for(i in 1:length(clonotype.list)){
    ...
    for(j in 1:nrow(clonotype.list[[i]])){
      ...
      temp_concat_ref_LC <- gsub(pattern = "_consensus_",replacement = "_concat_ref_",x = contig.list[[i]]$raw_consensus_id[which(contig.list[[i]]$raw_clonotype_id==clonotype.list[[i]]$clonotype_id[j] & contig.list[[i]]$is_cell=="True" & contig.list[[i]]$chain!="IGH")][1])
      ...
      VDJ.per.clone[[i]][[j]] <- data.frame(barcode=rep("",temp.cell.number),clonotype_id=rep(clonotype.list[[i]]$clonotype_id[j],temp.cell.number),isotype_hc=rep("",temp.cell.number),isotype_lc=rep("",temp.cell.number),
                                            ...
                                            trimmed_LC_sequence=rep("",temp.cell.number), trimmed_HC_sequence=rep("",temp.cell.number),umi_count=rep("",temp.cell.number),contig_id_hc=rep("",temp.cell.number),contig_id_lc=rep("",temp.cell.number),stringsAsFactors = F)
      ...
      for(k in 1:length(temp_str_split)){
        temp_barcode_list[[k]] <- as.character(unique(contig.list[[i]]$barcode[which(contig.list[[i]]$raw_clonotype_id==temp_str_split[k] & contig.list[[i]]$productive=="True")]))
      }
      VDJ.per.clone[[i]][[j]]$barcode <- unlist(temp_barcode_list)

      #### now need to be flexible with the barcodes
      for(k in 1:length(VDJ.per.clone[[i]][[j]]$barcode)){
        VDJ.per.clone[[i]][[j]]$isotype_hc[k] <- names(which.max(table(contig.list[[i]]$c_gene[which(contig.list[[i]]$is_cell=="True" & contig.list[[i]]$chain=="IGH" & contig.list[[i]]$barcode==VDJ.per.clone[[i]][[j]]$barcode[k])])))
        VDJ.per.clone[[i]][[j]]$isotype_lc[k] <- names(which.max(table(contig.list[[i]]$c_gene[which(contig.list[[i]]$is_cell=="True" & contig.list[[i]]$chain!="IGH" & contig.list[[i]]$barcode==VDJ.per.clone[[i]][[j]]$barcode[k])])))
        VDJ.per.clone[[i]][[j]]$umi_count[k] <- names(which.max(table(contig.list[[i]]$umis[which(contig.list[[i]]$is_cell=="True" & contig.list[[i]]$chain!="IGH" & contig.list[[i]]$barcode==VDJ.per.clone[[i]][[j]]$barcode[k])])))
        VDJ.per.clone[[i]][[j]]$contig_id_lc[k] <- names(which.max(table(contig.list[[i]]$contig_id[which(contig.list[[i]]$is_cell=="True" & contig.list[[i]]$chain!="IGH" & contig.list[[i]]$barcode==VDJ.per.clone[[i]][[j]]$barcode[k])])))
        VDJ.per.clone[[i]][[j]]$contig_id_hc[k] <- names(which.max(table(contig.list[[i]]$contig_id[which(contig.list[[i]]$is_cell=="True" & contig.list[[i]]$chain=="IGH" & contig.list[[i]]$barcode==VDJ.per.clone[[i]][[j]]$barcode[k])])))

        VDJ.per.clone[[i]][[j]]$HC_vgene[k] <- names(which.max(table(contig.list[[i]]$v_gene[which(contig.list[[i]]$is_cell=="True" & contig.list[[i]]$chain=="IGH" & contig.list[[i]]$barcode==VDJ.per.clone[[i]][[j]]$barcode[k])])))
        VDJ.per.clone[[i]][[j]]$HC_jgene[k] <- names(which.max(table(contig.list[[i]]$j_gene[which(contig.list[[i]]$is_cell=="True" & contig.list[[i]]$chain=="IGH" & contig.list[[i]]$barcode==VDJ.per.clone[[i]][[j]]$barcode[k])])))
        VDJ.per.clone[[i]][[j]]$LC_vgene[k] <- names(which.max(table(contig.list[[i]]$contig_id[which(contig.list[[i]]$is_cell=="True" & contig.list[[i]]$chain!="IGH" & contig.list[[i]]$barcode==VDJ.per.clone[[i]][[j]]$barcode[k])])))
        VDJ.per.clone[[i]][[j]]$LC_jgene[k] <- names(which.max(table(contig.list[[i]]$contig_id[which(contig.list[[i]]$is_cell=="True" & contig.list[[i]]$chain!="IGH" & contig.list[[i]]$barcode==VDJ.per.clone[[i]][[j]]$barcode[k])])))
