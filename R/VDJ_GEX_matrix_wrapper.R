...
VDJ_GEX_matrix <- function(VDJ.out.directory.list,
  ...
  #making sure that all helper functions are available 
  if(VDJ.out.directory.list[[1]] != "none"){
    if(!exists("barcode_VDJ_iteration")){stop("barcode_VDJ_iteration function not avialable. Check package installation or load function manually")}}
  if(GEX.out.directory.list[[1]] != "none"){
    if(!exists("GEX_automate_single")){stop("GEX_automate_single function not avialable. Check package installation or load function manually")}}
  if(get.VDJ.stats == T){
    if(!exists("VDJ_GEX_stats")){stop("VDJ_GEX_stats function not avialable. Check package installation or load function manually")}}
