...
#### Functions definition ####
        ...
        for(cel in 1:nrow(curr_ab)){
          ...
          if(length(clone_matches) > 0){
            ...
            #Directo  homology criterion
              ...
              } else { curr_ab$new_clonal_feature[cel] <- paste0("hclust_nomatch_", runif(1, 1, 1000000))}
          }
        }
        sample_aberrant <- curr_ab #re transpose into list
      }
  ...
  VDJ$Nr_of_VJ_chains[stringr::str_detect(VDJ$VJ_cdr3s_aa, ";")] <- 2
      ...
      for(i in 1:length(sample_dfs)){ 
        ...
        clones_g <- as.data.frame(sample_dfs[[i]] %>% group_by(new_clonal_feature) %>% summarise(new_clonal_frequency = n()))
  ...
  if(!global.clonotype){
    if(any(stringr::str_detect(names(VDJ.GEX.matrix), paste0("clonotype_id_",clone.strategy.as.input)))){
      ...
      VDJ.GEX.matrix <- VDJ.GEX.matrix[,-c(which(names(VDJ.GEX.matrix) %in% c(paste0("clonotype_id_",clone.strategy.as.input), paste0("clonal_feature_",clone.strategy.as.input), paste0("clonotype_frequency_",clone.strategy.as.input))))]
    }
    ...
    if(any(stringr::str_detect(names(VDJ.GEX.matrix), paste0("global_clonotype_id_",clone.strategy.as.input)))){
      VDJ.GEX.matrix <- VDJ.GEX.matrix[,-c(which(names(VDJ.GEX.matrix) %in% c(paste0("global_clonotype_id_",clone.strategy.as.input), paste0("global_clonal_feature_",clone.strategy.as.input), paste0("global_clonotype_frequency_",clone.strategy.as.input))))]
      message("Removing columns from VDJ.GEX.matrix stemming from a previous clonotyping run with same setting")
    }
