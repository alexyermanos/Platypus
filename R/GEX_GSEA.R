...
GEX_GSEA <- function(GEX.cluster.genes.output, MT.Rb.filter, filter, path.to.pathways, metric.colname, pval.adj.cutoff, Enrichment.Plots, my.own.geneset, platypus.version){
  if (missing(filter)) {
    filter <- c("MT-", "RPL", "RPS")
