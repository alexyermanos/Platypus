#'The VGM_expand_featurebarcodes function replaces the original sample_id column of a vgm object with a pasted version of the original sample_id and the last digits of the feature barcode.
#'This function can be used to trace back the cell origin of each sample after using cell hashing for single-cell sequencing. 
#'
...
#'
#'
#'@params vgm VGM output of VDJ_GEX_matrix function (Platypus V3)
#'@params by.majority.barcodes Logical. Default is FALSE. Indicated whether strict barcode assignment or majority barcode assignment should be used to create the new sample_id. If TRUE, for each clonotype the most frequent feature barcode will be chosen and assigned to each cell, even if that cell itself does not have this particular barcode assigned.
#'@params integrate.in.gex Logical. Default is FALSE. If TRUE, the newly created sample_id's are integrated into gex component as well. Not recommended if no further gex analysis is done due to much longer computational time. 
#'@params vdj.only Logical. Defines if only vdj information is provided as input. Default is set to FALSE. If set to TRUE a vdj dataframe has to be provided as input (vgm = vdj). Also, integrate.in.gex is automatically set to FALSE since no gex (vgm[[2]]) information is provided. 
#'@params platypus.version This function works with "v3" only, there is no need to set this parameter.
...
#'
#' # 1. If only vdj data (vgm[[1]]) and strict feature barcode assignment is used: 
#' vgm_expanded_fb <- VGM_expand_featurebarcodes(vgm = VDJ_GEX_matrix.output[[1]], by.majority.barcodes = FALSE, integrate.in.gex=FALSE, vdj.only= TRUE)
#' 
#' # 2. If whole vgm and strict fb assignment is used (gex and vdj - necessary if gene expression analysis of sub-samples is desired):
#' vgm_expanded_fb <- VGM_expand_featurebarcodes(vgm = VDJ_GEX_matrix.output, by.majority.barcodes = FALSE, integrate.in.gex=TRUE, vdj.only= FALSE)
#' 
#' # 3. If whole vgm and majority barcode assignement is used (gex and vdj - necessary if gene expression analysis of sub-samples is desired):
#' vgm_expanded_fb <- VGM_expand_featurebarcodes(vgm = VDJ_GEX_matrix.output, by.majority.barcodes = TRUE, integrate.in.gex=TRUE, vdj.only= FALSE)
#' 
#' #Note: Majority barcode assignment is recommended if the assumption that all cells within one clonotype originate from the same sample sub-group is feasible. 
#' 
#'}
