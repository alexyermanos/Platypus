...
#' @param vdj.gex.matrig.output If Platypus v3 is used, the input to this function has to be the output of the VDJ_GEX_matrix function.
...
#' 
#' # Version 2
#'    vdj_repertoire_tcells <- VDJ_analyze(VDJ.out.directory =VDJ.out.directory.list, filter.1HC.1LC = T)
#'    gex_acute <- Platypus::GEX_automate(GEX.outs.directory.list = dir_to_gex[1:1], integration.method = "scale.data", mito.filter = 20, cluster.resolution = 0.5, VDJ.gene.filter = T)
#'
#' clonotyme_output <- VDJ_GEX_clonotyme(vdj.analyze.output = vdj_repertoire_tcells, gex.analyze.output = gex_acute, version="v2", exclude.clusters=7, highlight.genes="sell", colors = c("blue", "red", "black", "orange")
#' clonotyme_output[[4]]
#' 
#' # Version 3
#'     VGM <- readRDS("C:/Users/rapha/Downloads/TEMPLATE_VDJ_GEX_mat_Bcells_r2_150521.rds")
#'     
#' clonotyme_output <- VDJ_GEX_clonotyme(vdj.gex.matrix.output = VGM, version="v3", highlight.genes="sell", top.N.clonotypes = 1)
#' 
#'}
#'
#'
VDJ_GEX_clonotyme <- function(method, version, top.N.clonotypes, vdj.gex.matrix.output, vdj.analyze.output, gex.automate.output, exclude.clusters, colors, show.cells, highlight.genes, dropest.output.list){ #, velocyto.gex.merged, velocyto.topN.per.sample, velocyto.out.dir, velocyto.save.rds, velocyto.norm.scale.factor, velocyto.n.variable.features, velocyto.neighbor.dim,velocyto.cluster.resolution, velocyto.mds.dim, velocyto.nCount_spliced, velocyto.percent.mt, velocyto.normalisation.method, velocyto.selection.method
  ...
  if(method == "monocle3"){    
      ...
      if(version=="v3"){
        ...
        number.of.samples <- length(na.omit(unique(vdj.gex.matrix.output[[2]]$sample_id)))
        ...
        colData(Cell.Data.Set)$barcodes <- rownames(colData(Cell.Data.Set)) #adding barcodes column to Cell.Data.Set
        ...
        colData(Cell.Data.Set)$partitions <- Cell.Data.Set@clusters$UMAP$partitions
        ...
        colData(Cell.Data.Set)$clonotype <- "other"
        ...
        for (k in 1:number.of.samples){
          for (i in 1:top.N.clonotypes){
            ...
            for(j in 1:length(temp_barcodes)){
              if(length(which(colData(Cell.Data.Set)$barcodes==temp_barcodes[j]))){
                index[j]<- which(colData(Cell.Data.Set)$barcodes==temp_barcodes[j])
              }
            }
            ...
            colData(Cell.Data.Set)$clonotype[index] <- paste0("Sample ",k," Clonotype ", i)
      ...
      colData(Cell.Data.Set)$pseudotime <- pseudotime(Cell.Data.Set, reduction_method = "UMAP")
      
      print("Preparing Ridgeline Plots...")
      df <- as.data.frame(colData(Cell.Data.Set))
      ...
      if(missing(colors)){
        colors <- rainbow(top.N.clonotypes*number.of.samples)
        cols <- c(colors, "grey")
      }else{
      ...
      if(show.cells){
        ridgeline.plot <- ggplot(df, aes(x = pseudotime, y =clonotype,fill=clonotype, alpha=0.5))+ geom_density_ridges(aes(point_size=p.size, point_shape=p.shape),scale=2, jittered_points = TRUE, point_alpha=1)+theme_classic()+scale_fill_manual(values=cols)+scale_point_size_continuous(range = c(0, 2), guide = "none")+scale_discrete_manual(aesthetics = "point_shape", values = c(21, NA))
      }else{
        ridgeline.plot <- ggplot(df, aes(x = pseudotime, y =clonotype,fill=clonotype))+ geom_density_ridges(scale=2, jittered_points = F, point_alpha=1)+theme_classic()+scale_fill_manual(values=cols)
      }
      ...
      rowData(Cell.Data.Set)$gene_short_name <- rownames(rowData(Cell.Data.Set))
      Cell.Data.Set <- estimate_size_factors(Cell.Data.Set)
      
      print(paste0("Selecting genes: ", unlist(highlight.genes)))
      fData(Cell.Data.Set)$gene_short_name_toupper <- toupper(fData(Cell.Data.Set)$gene_short_name)
      genes <- row.names(subset(fData(Cell.Data.Set), gene_short_name_toupper %in% toupper(highlight.genes))) 
      ...
      gene.plot.clonotype <- ggplot(aes(pseudotime, expression), data = gene.plot.cluster$data) + geom_point(aes(color = clonotype, size=clonotype, alpha=clonotype))#, position = position_jitter(horizontal_jitter, vertical_jitter)
      gene.plot.clonotype <- gene.plot.clonotype + geom_line(aes(x = pseudotime, y = expectation), 
                                                             data = gene.plot.cluster$data)
      gene.plot.clonotype <- gene.plot.clonotype + scale_y_log10() + facet_wrap(~feature_label, nrow = length(highlight.genes), 
                                                                                ncol = 1, scales = "free_y")
      gene.plot.clonotype <- gene.plot.clonotype + expand_limits(y = c(0, 1))
      gene.plot.clonotype <- gene.plot.clonotype + ylab("Expression")
      gene.plot.clonotype <- gene.plot.clonotype + xlab("Pseudotime")
      gene.plot.clonotype <- gene.plot.clonotype + theme_minimal()
      gene.plot.clonotype <- gene.plot.clonotype + scale_color_manual(breaks = c(level.list, "other"), values=cols) + scale_size_manual(breaks = c(level.list, "other"), values=c(2,2,2,2,1.5)) + scale_alpha_manual(breaks = c(level.list, "other"), values=c(0.7,0.7,0.7,0.7,0.4))#+theme(legend.position = "bottom", legend.box="vertical", legend.margin=margin())
  ...
  if(method=="velocyto"){
    ...
    # if(missing(dropest.output.list)){print("Please provide Exond and Intron alignments by DropEst: cell.counts.matrices.rds required as Input.")}
    ...
    #       gex <- as.Seurat(x=cell.count.mat)
    #       gex[["spliced"]]<- CreateAssayObject( counts = cell.count.mat$exon)
    #       gex[["unspliced"]] <- CreateAssayObject( counts = cell.count.mat$intron)
    #       gex[["spanning"]] <- CreateAssayObject( counts = cell.count.mat$spanning)
    #       gex$sample <- sample
    #       return(gex)
    #     }
    #     
    ...
    #       
    #       obj <- NormalizeData(obj, normalization.method = velocyto.normalisation.method, scale.factor = velocyto.norm.scale.factor)
    #       obj <- FindVariableFeatures(obj, selection.method = velocyto.selection.method, nfeatures = velocyto.n.variable.features)
    #       all.genes <- rownames(obj)
    #       obj <- ScaleData(obj, features = all.genes)
    #       obj <- RunPCA(obj, features = Seurat::VariableFeatures(object = obj))
    #       obj <- FindNeighbors(obj, dims = velocyto.neighbor.dim)
    #       obj <- FindClusters(obj, resolution = velocyto.cluster.resolution)
    #       obj <- RunUMAP(obj, dims = velocyto.mds.dim)
    #       return(obj)
    #     }
    #     
    #     #####
    #     
    #     Velocyto_sample_run <- function(sample, file.name, velocyto.out.dir, velocyto.save.rds, velocyto.deltaT, velocyto.kCells, velocyto.fit.quantile){
    #       ident.colors <- (scales::hue_pal())(n = length(x = levels(x = sample)))
    #       names(x = ident.colors) <- levels(x = sample)
    #       cell.colors <- ident.colors[Idents(object = sample)]
    #       names(x = cell.colors) <- colnames(x = sample)
    #       sample <- RunVelocity(object = sample, deltaT = velocyto.deltaT, kCells = velocyto.kCells, fit.quantile = velocyto.fit.quantile)
    #       
    ...
    #     #####
    #     
    #     Velocyto_sample_plots <- function(sample, file.name, velocyto.out.dir){ 
    #       ident.colors <- (scales::hue_pal())(n = length(x = levels(x = sample)))
    #       names(x = ident.colors) <- levels(x = sample)
    #       cell.colors <- ident.colors[Idents(object = sample)]
    #       names(x = cell.colors) <- colnames(x = sample)
    #       
    #       pdf(paste0(velocyto.out.dir,"/",file.name,'.plot.pdf'), height=5, width=6)
    #       show.velocity.on.embedding.cor(emb = Embeddings(object = sample, reduction = "umap"), vel = Tool(object = sample, slot = "RunVelocity"), n = 200, scale = "sqrt", cell.colors = ac(x = cell.colors, alpha = 0.5), cex = 0.8, arrow.scale = 3, show.grid.flow = TRUE, min.grid.cell.mass = 0.5, grid.n = 40, arrow.lwd = 1, do.par = FALSE, cell.border.alpha = 0.1)
    #       dev.off()
    #       
    #       FeaturePlot(sample, highlight.genes, order=T)
    #       ggsave(paste0(velocyto.out.dir,"/",file.name,'.feature.plot.pdf'), height=5, width=6)
    #       
    #       DimPlot(sample, reduction="umap")
    #       ggsave(paste0(velocyto.out.dir,"/",file.name,'.umap.pdf'), height=5, width=6)
    #       
    #       return(sample)
    #     }
    #     
    #     #####
    #     
    #     Velocyto_pseudotime_plots <- function(sample, file.name, gene, velocyto.out.dir, velocyto.deltaT, velocyto.kCells, velocyto.fit.quantile, velocyto.kGenes){ 
    ...
    #       cell.colors <- ident.colors[Idents(object = sample)]
    #       
    #       pdf(paste0(velocyto.out.dir,"/",file.name,'_', gene ,'.gene.pdf'), height =7, width=25)
    #       fit.quantile<-0.02
    #       gene.relative.velocity.estimates(
    #         GetAssayData(sample, slot = "data", assay = "spliced"),
    #         GetAssayData(sample, slot = "data", assay = "unspliced"),
    #         cell.emb = Embeddings(sample, "umap"),
    #         deltaT=velocyto.deltaT,kCells = velocyto.kCells,kGenes=velocyto.kGenes,
    #         show.gene = gene,
    #         old.fit = Tool(sample, slot = "RunVelocity"),
    #         do.par=T,
    ...
    #     }
    #     
    #     Velocyto_highlight_clonotypes <- function(sample, barcodes_vdj, clonotype.number){
    #       
    ...
    #         for(m in 1:nrow(barcodes_vdj[[k]])){
    #           barcodes[[k]][[m]] <- stringr::str_split(barcodes_vdj[[k]]$barcodes[m], ";")
    ...
    #       }
    #       
    #       Idents(sample) <- sample$clonotype
    #       
    #       ident.colors <- (scales::hue_pal())(n = length(x = levels(x = sample)))
    #       names(x = ident.colors) <- levels(x = sample)
    ...
    #       for(k in 1:length(barcodes)){
    #         for(m in 1:length(barcodes[[k]])){
    #             ident.colors[which(names(ident.colors)=="Clonotype_", m," Mouse_", k)]<- colors[((k-1)*length(barcodes[[k]])+m)]
    #         }
    #       }
    #       
    #       t1 <- head(ident.colors,1)
    #       t1 <- ac(t1, alpha=0.1)
    #       t2 <- tail(ident.colors,(length(ident.colors)-1))
    #       t2 <- ac(t2, alpha=1)
    ...
    #       names(x = cell.colors) <- colnames(x = sample)
    #       
    #       pdf(paste0(velocyto.out.dir,"/",file.name,"_",sample.name,'.clonotypes.on.velocyto.plot.pdf'), height=5, width=6)
    #       show.velocity.on.embedding.cor(emb = Embeddings(object = sample, reduction = "umap"), vel = Tool(object = sample, slot = "RunVelocity"), n = 200, scale = "sqrt", cell.colors = cell.colors, cex = 0.8, arrow.scale = 3, show.grid.flow = TRUE, min.grid.cell.mass = 0.5, grid.n = 40, arrow.lwd = 1, do.par = FALSE, cell.border.alpha = 0)
    #       dev.off()
    #       
    #     }
    #     
    #     ####
    #     
    #     Velocyto_Wrapper <- function(gex, highlight.genes, file.name, velocyto.save.rds, exclude.clusters, velocyto.out.dir, vdj_repertoire, velocyto.norm.scale.factor, velocyto.n.variable.features, velocyto.neighbor.dim,velocyto.cluster.resolution, velocyto.mds.dim, velocyto.nCount_spliced, velocyto.percent.mt, velocyto.normalisation.method, velocyto.selection.method, velocyto.deltaT, velocyto.kCells, velocyto.fit.quantile, velocyto.kGenes){
    #       
    ...
    #       
    #       gex <- Velocyto_sample_run(sample = gex, file.name=file.name, velocyto.out.dir = velocyto.out.dir, velocyto.save.rds = velocyto.save.rds, velocyto.deltaT = velocyto.deltaT, velocyto.kCells = velocyto.kCells, velocyto.fit.quantile=velocyto.fit.quantile)
    #       gex <- Velocyto_sample_plots(sample = gex, file.name=file.name, velocyto.out.dir = velocyto.out.dir, velocyto.save.rds = velocyto.save.rds)
    #       gex <- Velocyto_pseudotime_plots(sample = gex, file.name=file.name, gene = highlight.genes, velocyto.out.dir = velocyto.out.dir, velocyto.save.rds = velocyto.save.rds)
    #       
    #       gex <- Velocyto_highlight_clonotypes(sample = gex, vdj_repertoire = vdj_repertoire)
    #       
    #       return(gex)
    #     }
    #     
    ...
    #         barcodes[[i]] <- head(vdj_repertoire_tcells[[i]]$barcodes, top.N.clonotypes)
    #       }
    #     }else if(version == "v3"){
    ...
    #         barcodes[[i]] <- head(filter(vdj_repertoire_tcells$sample_id == paste0('s', i))$barcodes, top.N.clonotypes)
    #       }
    #     }else{
