...
#' #module load gcc/4.8.5 r/4.0.2
#' #bsub -W 2880 -R 'rusage[mem=20000]' /cluster/home/rakuhn/dropEst/dropest -V -C 6000 -f -g /cluster/scratch/rakuhn/mm10-2020-A/refdata-gex-mm10-2020-A/genes/genes.gtf -c /cluster/home/rakuhn/dropEst/configs/10x.xml /cluster/scratch/rakuhn/cellranger_v5/g1/outs/possorted_genome_bam.bam
#' 
#' #Load required VDJ.analyze.output on EULER CLUSTER
#' 
#' vdj_repertoire_tcells <- readRDS("/cluster/home/rakuhn/RPII/vdj_repertoire_tcells.rds")
#' vdj_repertoire_tcells <- head(vdj_repertoire_tcells,2)  #Only select the first two repertoires since we only want to analyze these two.
#' 
#' # Load the two corresponding Dropest cell.count.matrices.rds
#' 
#' dropest.output.list <- list()
#' dropest.output.list[[1]] <- readRDS("/cluster/home/rakuhn/RPII/old_bam/gex1/cell.counts.matrices.rds")
#' dropest.output.list[[2]] <- readRDS("/cluster/home/rakuhn/RPII/old_bam/gex2/cell.counts.matrices.rds")
#' 
#' # Run Velocyto using Clonotyme
#' 
#' VDJ_GEX_clonotyme(method = "velocyto", version = "v2", vdj.analyze.output = vdj_repertoire_tcells, dropest.output.list = dropest.output.list, top.N.clonotypes = 3, exclude.clusters = 8, highlight.genes = "sell", velocyto.gex.merged = T, velocyto.out.dir = ".", velocyto.save.rds = F)
#' 
#'}
#'
#'
  ...
  if(method=="velocyto"){
    ...
    if(operating.system=="Windows"){
      ...
      Velocyto_sample_plots <- function(sample, sample.name, file.name, velocyto.out.dir){
        ...
        ggsave(paste0(velocyto.out.dir,"/",file.name,"_",sample.name,'.feature.plot.pdf'), height=5, width=6)
        
        DimPlot(sample, reduction="umap")
        ggsave(paste0(velocyto.out.dir,"/",file.name,"_",sample.name,'.umap.pdf'), height=5, width=6)
      ...
      Velocyto_pseudotime_plots <- function(sample, file.name, sample.name, gene, velocyto.out.dir, velocyto.deltaT, velocyto.kCells, velocyto.fit.quantile, velocyto.kGenes){
        ...
        pdf(paste0(velocyto.out.dir,"/",file.name,"_",sample.name,'_', gene ,'.gene.pdf'), height =7, width=25)
        fit.quantile<-0.02
