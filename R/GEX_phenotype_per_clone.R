...
#' @param GEX.group.by For platypus.version == "v3". Character. Column name of the GEX.matrix@meta.data to group barplot by. Defaults to seurat.clusters
#' @param GEX.clonotypes For platypus.version == "v3". Numeric vector with ids of clonotypes to plot e.g. c(1,2,3,4) 
...
print(class(temp.matrix))
  ...
  return(stacked.ggplot)
    ...
    desired_strings <- paste0("clonotype",GEX.clonotypes)
    ...
    temp.melt$clonotype_id <- ordered(as.factor(temp.melt$clonotype_id), levels = paste0("clonotype",GEX.clonotypes))
