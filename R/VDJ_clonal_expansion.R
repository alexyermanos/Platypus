...
#' @param group.by Character. Defaults to "sample_id". Column name of VDJ.matrix to split VDJ.matrix by. For each unique entry in that column a plot will be generated. Therefore plots can be generated by sample_id, group_id or any other metadata item. 
#' @param color.by Character. Defaults to "isotype". If set to "isotype" bars are colored by the respective IgH chain or in grey for T cells. This can alternatively be set to any column name of the VDJ.matrix. This allows coloring clones by their V_gene usage or by GEX clusters
...
#' @return Returns a nested list. out[[1]] are plots, out[[2]] are dataframes with additional barcode and CDR3 information
#' @export
#' @examples
#' \dontrun{
#' 
...
#'VDJ_isotypes_per_clone(VDJ.matrix = VDJ.GEX.matrix.out[[1]], celltype = "Bcells", clones = 30,subtypes = F, species = "Mouse",treat.incomplete.clones = "exclude",treat.incomplete.cells = "proportional", group.by = "seurat_clusters", color.by = "sample_id")
#'
#'#T cell plot with recoloring by vgene
#'VDJ_isotypes_per_clone(VDJ.matrix = VDJ.GEX.matrix.out[[1]], celltype = "Tcells", clones = 30, group.by = "sample_id", color.by = "VDJ_vgene")
#'
#'}
VDJ_clonal_expansion <- function(VDJ.matrix,
  ...
  if(missing(group.by)){
    group.by <- "sample_id"}
  if(!group.by %in% names(VDJ.matrix) & group.by != "sample_id"){
  ...
  if(celltype == "Bcells"){ ####START Bcells
    ...
    if(subtypes == FALSE){
      ...
      for (i in 1:length(sample.names)){
        ...
        if(color.by != "isotype"){
          curr_rep_iso$colors <- VDJ.matrix[,color.by]
        }
        ...
        output_plot[[i]] <- ggplot2::ggplot(clones_per_isotype_all[[i]], ggplot2::aes(fill = Isotype, y=Counts, x=ClonalRank)) + ggplot2::geom_bar(stat="identity", width=0.6, color="black") + ggplot2::theme_bw() + ggplot2::scale_fill_manual(values = c("IGHG" = "green4", "IGHM" = "black", "IGHA" = "red3", "IGHD"="blue", "IGHE"="purple", "Unknown"="gray")) + ggplot2::theme_classic() + ggplot2::ggtitle(paste0(i)) + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) + ggplot2::scale_y_continuous(expand = c(0,0)) + ggplot2::scale_x_continuous(expand = c(0,0.5)) + ggplot2::labs(title = sample.names[[i]], x = "Clonal rank", y = "Number of cells")
          ...
          output_plot[[i]] <- ggplot2::ggplot(clones_per_isotype_all[[i]], ggplot2::aes(fill = Isotype, y=Counts, x=ClonalRank)) + ggplot2::geom_bar(stat="identity", width=0.6, color="black") + ggplot2::theme_bw() + ggplot2::theme_classic() + ggplot2::ggtitle(paste0(i)) + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) + ggplot2::scale_y_continuous(expand = c(0,0)) + ggplot2::scale_x_continuous(expand = c(0,0.5)) + ggplot2::labs(title = sample.names[[i]], x = "Clonal rank", y = "Number of cells", fill = color.by)
    ...
    if(subtypes == TRUE){
      ...
      for (i in 1:length(sample.names)){
        ...
        if(color.by != "isotype"){
          output_plot[[i]] <- ggplot2::ggplot(clones_per_isotype_all[[i]], ggplot2::aes(fill = Isotype, y=Counts, x=ClonalRank)) + ggplot2::geom_bar(stat="identity", width=0.6, color="black") + ggplot2::theme_bw() + ggplot2::theme_classic() + ggplot2::ggtitle(paste0(i)) + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) + ggplot2::scale_y_continuous(expand = c(0,0)) + ggplot2::ylab("Number of cells") + ggplot2::xlab("Clonal rank") +ggplot2::labs(fill = color.by) + ggplot2::scale_x_continuous(expand = c(0,0.5))
        } else {
        if (species == "Human"){
      ...
      for (i in 1:length(sample.names)){
        ...
        for (j in 1:clones){
          ...
          if(nrow(curr_clone) > 0){
            ...
            clones_per_isotype[[j]] <- data.frame("Counts"=rep(0, 1), "Color"=rep("", 1), "Isotype"=rep("", 1), "ClonalRank"=rep("", 1), "clonotype_id" = rep(curr_clone$clonotype_id_10x[1],1), "VDJ_cdr3s_aa" = rep(curr_clone$VDJ_cdr3s_aa[which(curr_clone$VDJ_cdr3s_aa != "")][1],1), "VJ_cdr3s_aa" = rep(curr_clone$VJ_cdr3s_aa[which(curr_clone$VJ_cdr3s_aa != "")][1],1), "barcode" = rep(paste0(curr_clone$barcode, collapse = ";"),1))  #to maintain clonotype information
            
            clones_per_isotype[[j]]$Counts[1] <- sum(stringr::str_count(curr_clone$isotype, "none"))

            if(color.by == "isotype"){
              clones_per_isotype[[j]]$Color <- c("gray80")
        ...
        if(color.by == "isotype"){
          output_plot[[i]] <- ggplot2::ggplot(clones_per_isotype_all[[i]], ggplot2::aes(fill = Isotype, y=Counts, x=ClonalRank)) + ggplot2::geom_bar(stat="identity", width=0.6, color="black") + ggplot2::theme_bw() + ggplot2::scale_fill_manual(values = c("gray80")) + ggplot2::theme_classic() + ggplot2::ggtitle(paste0(i)) + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) + ggplot2::scale_y_continuous(expand = c(0,0)) + ggplot2::scale_x_continuous(expand = c(0,0.5)) + ggplot2::labs(title = sample.names[[i]], x = "Clonal rank", y = "Number of cells")
        } else {
          output_plot[[i]] <- ggplot2::ggplot(clones_per_isotype_all[[i]], ggplot2::aes(fill = Isotype, y=Counts, x=ClonalRank)) + ggplot2::geom_bar(stat="identity", width=0.6, color="black") + ggplot2::theme_bw() + ggplot2::theme_classic() + ggplot2::ggtitle(paste0(i)) + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) + ggplot2::scale_y_continuous(expand = c(0,0)) + ggplot2::scale_x_continuous(expand = c(0,0.5)) + ggplot2::labs(title = sample.names[[i]], x = "Clonal rank", y = "Number of cells", fill = color.by)
