#' Clonal frequency plot. The isotype usage of each clone will be indicated in the case of B cells
...
#' @examples
#' \dontrun{
#' VDJ.clonal.expansion <- VDJ_clonal_expansion(VDJ_clonotype_output = VDJ.analyze.output, VDJ_per_clone_output = VDJ.per.clone.output, clones = 30)
#' version3 <- VDJ_clonal_expansion(VDJ.GEX.matrix.out  = VDJ_GEX_matrix[[1]], subtypes = T, species = "Mouse", treat.incomplete.clones = "exclude", treat.incomplete.cells = "proportional", clones = 30,platypus.version = "v3")
#'}
VDJ_clonal_expansion <- function(VDJ_clonotype_output,
  ...
  if(platypus.version=="v3"){ ####START v3
      ...
      if(class(VDJ.GEX.matrix.out) == "data.frame"){ #converting a single dataframe intput into a list for compatibility
