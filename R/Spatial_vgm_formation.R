#' Utility to add spatial information to VGM
#' @description Addition of the spatial information to the VGM matrix, output of VDJ_GEX_matrix()
...
#' @param tissue_positions_list_path Path to a text file containing a table with rows that correspond to spots in csv format
...
Spatial_vgm_formation<-function(vgm,
  ...
  for (i in 1:length(sample_names)){
    ...
    vgm$spatial$matrix[[i]]<-as.data.frame(t(Seurat::Read10X_h5(matrix_path[[i]])))
  }
  return(vgm)
}
