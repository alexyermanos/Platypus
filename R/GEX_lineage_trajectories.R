...
GEX_lineage_trajectories <- function( GEX,
  ...
  invisible(lines(slingshot::SlingshotDataSet(sds), lwd = 1, type = 'lineages', col = 'black'))
  lineage_plot= invisible(recordPlot())
  
  invisible(plot(SingleCellExperiment::reducedDims(sds)$UMAP, col = cell_colors_clust, pch = 16, cex = 0.5))
  invisible(lines(slingshot::SlingshotDataSet(sds), lwd = 1,  col = 'black'))
  lineage_line_plot = invisible(recordPlot())
