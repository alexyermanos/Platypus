  ...
  if(antigen.interaction){
    ...
    for(i in 1:length(vis.structure)){
      ...
      if(SASA){
        ...
        m_SASA_A <- SASA_df %>% filter(chain == "A" & resno %in% res_A) %>% .$SASA.prot %>% mean()
        m_SASA_B <- SASA_df %>% filter(chain == "B" & resno %in% res_B) %>% .$SASA.prot %>% mean()
        m_SASA_C <- SASA_df %>% filter(chain == "C" & resno %in% res_C) %>% .$SASA.prot %>% mean()
      ...
      if(hydrophobicity){
        ...
        m_Hydph_A <- df_hydph %>% filter(chain == "A" & resno %in% res_A) %>% .$hydrophobicity %>% mean()
        m_Hydph_B <- df_hydph %>% filter(chain == "B" & resno %in% res_B) %>% .$hydrophobicity %>% mean()
        m_Hydph_C <- df_hydph %>% filter(chain == "C" & resno %in% res_C) %>% .$hydrophobicity %>% mean()
      ...
      if(charge){
        ...
        m_charge_A <- df_charge %>% filter(chain == "A" & resno %in% res_A) %>% .$charge %>% mean()
        m_charge_B <- df_charge %>% filter(chain == "B" & resno %in% res_B) %>% .$charge %>% mean()
        m_charge_C <- df_charge %>% filter(chain == "C" & resno %in% res_C) %>% .$charge %>% mean()
    ...
    if(binding.residue.barplot){
      ...
      if(binding.residue.barplot.style == "all_regions" | binding.residue.barplot.style == "both"){
        ...
        # Basic barplot
        bar_plot[[pp]] <-ggplot2::ggplot(data=bin.res.pp, aes(x=region, y=Nr_bind_res, fill=region)) +
          ggplot2::geom_bar(stat="identity") + 
          ggplot2::coord_flip() + 
          ggplot2::ylab("Nr. of residues in binding site") + 
          ggplot2::xlab("Region") + 
          ggplot2::ggtitle(cells.to.vis[[ceiling(pp/length(rank.list))]]) + 
          ggplot2::theme(plot.title = element_text(hjust = 0.5), legend.position = "none") + 
        ...
        # Basic barplot
        bar_plot <-ggplot2::ggplot(data=overall_df, aes(x=region, y=Sum_nr_bind_res_pct, fill=region)) +
          ggplot2::geom_bar(stat="identity") + 
