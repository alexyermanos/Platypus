...
Spatial_evolution_of_clonotype_plot<-function(simulation =c(TRUE,FALSE),AbForest_output,VDJ,nb_clonotype, simulated_VDJ,tracking_type = c("closest","all"),sample_names,bcs_merge,images_tibble,title,size,legend_title){
  ...
  if(simulation == FALSE){
    ...
    if(length(tree_pathway$from)==0){
      plot<-ggplot(data =available_cells, aes(x=x,y=y, fill = as.factor(label)))+
    ...
    if(tracking_type == "closest"){
      ...
      for (l in 1:length(unique(tree_pathway$from))) {
        ...
        for(k in 1:nb_daughter_cell){
          ...
          mother_cells<-filter(subset_available_cells,subset_available_cells$mother_or_daughter == "mother")
          mother_cells<-select(mother_cells, barcode,x,y)
          daughter_cells<-filter(subset_available_cells,subset_available_cells$mother_or_daughter == "daughter")
          daughter_cells<-select(daughter_cells, barcode, x,y)
      ...
      arrow_coordinates<-select(arrow_coordinates, barcode, x,y,label, Daughter)
      names(arrow_coordinates)<-c("mother","x0","y0","mother_label","barcode")
      arrow_coordinates<-merge(arrow_coordinates,available_cells)
      arrow_coordinates<-select(arrow_coordinates,mother,x0,y0,mother_label,barcode,x,y,label)
    ...
    plot<-ggplot(mapping = aes(x,y))+
      geom_spatial(data=images_tibble[1,], aes(grob=grob), x=0.5, y=0.5)+ #add spatial image
      xlim(0,max(bcs_merge %>% 
                   filter(sample ==sample_names[1]) %>% 
                   select(width)))+
      ylim(max(bcs_merge %>% 
      ...
      geom_point(data = available_cells, aes(x,y,fill=factor(distance_from_germline)),shape = 21, colour = "black", size = 1.5, stroke = 0.5)+ #cells with 0 SHM without germline
      scale_fill_discrete(guide = guide_legend(reverse=TRUE))+
      #geom_point(data = germline_cell, aes(x=x, y = y, fill = "germline"),shape = 21, colour = "black", size = 1.25, stroke = 0.5)+ #germline cell
      geom_segment(data = arrow_coordinates, aes(x = x0, y = y0, xend = x1, yend = y1), arrow = arrow(length = unit(0.01, "npc")),colour = "#FFFFFF")+
      guides(fill = guide_legend(override.aes = list(size=3)))+
      theme_set(theme_bw(base_size = 10))+
      labs(fill = "Number of SHM")+
      theme(panel.grid.major = element_blank(), 
            panel.grid.minor = element_blank(),
            panel.background = element_blank(), 
            axis.line = element_line(colour = "black"),
  ...
  if(simulation == TRUE){
    ...
    plot<-ggplot(mapping = aes(x,y))+
      ...
      theme(axis.text=element_text(size=15),
            axis.title=element_text(size=15))+
      geom_point(data = available_cells, aes(x,y,fill=factor(distance_from_germline)),shape = 21, colour = "black", size = 3, stroke = 0.5)+ #cells with 0 SHM without germline
      scale_fill_discrete(guide = guide_legend(reverse=TRUE))+
      #geom_point(data = germline_cell, aes(x=x, y = y, fill = "germline"),shape = 21, colour = "black", size = 1.25, stroke = 0.5)+ #germline cell
      geom_segment(data = arrow_coordinates, aes(x = x0, y = y0, xend = x1, yend = y1), arrow = arrow(length = unit(0.01, "npc")),colour = "#FFFFFF")+
      guides(fill = guide_legend(override.aes = list(size=3)))+
