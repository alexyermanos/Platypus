#' Spatial assignment for simulated data
...
Spatial_VDJ_assignment<-function(GEX_matrix,
  ...
  germline_or_clone<-as.data.frame(germline_or_clone)
  germline_or_clone<-bind_cols(germline,germline_or_clone)
  ...
  if (method == "density"){
    ...
    for (i in 1:length(clonotype$clonotype_id)){
      if (length(available_cells$x)!=1){
        ...
        proba_density_x<-density(available_cells$x)
        proba_x<- approxfun(proba_density_x$x, proba_density_x$y) #approximation function for density function
        ...
        #for y
        proba_density_y<-density(available_cells$y)
        proba_y<- approxfun(proba_density_y$x, proba_density_y$y) #approximation function for density function
        ...
        used_cells<-bind_cols(available_cells,proba_density_x,proba_density_y)
        ...
        a<-bind_cols(subset_used_cells,subset_available_VDJ)
        assignment_matrix<-bind_rows(assignment_matrix,a)
        ...
        subset_available_VDJ<-available_VDJ[available_VDJ$clonotype==clonotype$clonotype_id[[i]],]
        a<-bind_cols(available_cells,subset_available_VDJ)
        assignment_matrix<-bind_rows(assignment_matrix,a)
