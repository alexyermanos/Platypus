...
#'
#'
#'@params vgm output of VDJ_GEX_matrix function in platypus, first dataset (public cell/clone information will only be stored in vgm_1)
#'@params vdj_public vgm[[1]] of all public data used for comparison joined together (multiple datasets can be joined, no information is stored here)
#'@params define.public Character, defines which column(s) in the vgm[[1]] are used to define a public cell. Default is set to "VDJ_cdr3s_aa". Other options are: "VJ_cdr3s_aa", "VJ_cdr3s_aa.VDJ_cdr3s_aa", "VDJ_cdr3s_nt"
#'@params check.overlap.type Logical, defines if overlap type information should be stored in the new vgm. Default is set to FALSE, setting this to TRUE will increase computational time. (Information on cd3 overlaps on the aa level will be stored)
#'@params integrate.into.gex Logical, defines if the public cell/clone information should be integrated into the vgm[[[2]]. Default is to FALSE, if set to TRUE computational time increases significantly. 
#'@params vdj.only Locical, if set to TRUE a vdj dataframe can be used as input (vgm = vdj_dataframe). A vdj dataframe with integrated public information is returned instead of a new vgm. Default is FALSE
#'
