...
VDJ_GEX_overlay_clones <- function(GEX.output,
  ...
  if(by.sample == F & by.group == F){
    ...
    if(missing(clone.colors)) clone.colors <- grDevices::rainbow(length(unique(GEX.output@meta.data$cl_to_plot))-1)
    ...
    out.plot <- Seurat::DimPlot(GEX.output,reduction = reduction, group.by = "cl_to_plot", cols = c(clone.colors,"grey80"), shuffle = T, pt.size = pt.size) + ggplot2::labs(col = "Rank / Sample id / Clonotype / Frequency", title = "")
  ...
  if(by.sample == T){
    ...
    if(clones.to.plot == "none"){
      if(missing(clone.colors)) clone.colors <- rep(grDevices::rainbow(n.clones), length(unique(GEX.output@meta.data$sample_id)))
    ...
    #dimplot with cols + grey30 to color in the non selected clones
    out.plot <- Seurat::DimPlot(GEX.output,reduction = reduction, group.by = "cl_to_plot", cols = c(clone.colors,"grey80"), shuffle = F, pt.size = pt.size) + ggplot2::labs(col = "Rank / Sample id / Clonotype / Frequency", title = "") + ggplot2::facet_wrap(~GEX.output@meta.data$sample_id, ncol = ncol.facet)
  ...
  if(by.group == T){
    ...
    if(clones.to.plot == "none"){
      if(missing(clone.colors)) clone.colors <- rep(grDevices::rainbow(n.clones), length(unique(GEX.output@meta.data$group_id)))
    ...
    #dimplot with cols + grey30 to color in the non selected clones
    out.plot <- Seurat::DimPlot(GEX.output,reduction = reduction, group.by = "cl_to_plot", cols = c(clone.colors,"grey80"), shuffle = F, pt.size = pt.size) + ggplot2::labs(col = "Rank / Group id / Sample id / Clonotype / Frequency", title = "") + ggplot2::facet_wrap(~GEX.output@meta.data$group_id, ncol = ncol.facet)
    #! Bug in the current version of seurat: shuffle = T does not work with split_by or + facet_wrap(). The bug is reported and should be fixed soon (13.4.21) https://github.com/satijalab/seurat/issues/4300
