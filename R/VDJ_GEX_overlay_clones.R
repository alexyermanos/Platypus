...
#' @param GEX.output A single seurat object from VDJ_GEX_matrix, which also includes VDJ information in the metadata (set integrate.VDJ.to.GEX to TRUE in the VDJ_GEX_matrix function) In most cases this will be VDJ_GEX_matrix.output[[2]]
...
VDJ_GEX_overlay_clones <- function(GEX.output,
                                   reduction,
                                   n.clones,
                                   clones.to.plot,
                                   by.sample,
  ...
  if(by.other.group %in% names(GEX.output@meta.data)){
    by.group <- T
    GEX.output@meta.data$group_id <- GEX.output@meta.data[,by.other.group]
  } else{by.group <- F}
  
  
  if(!clones.to.plot %in% names(GEX.output@meta.data) & clones.to.plot != "none"){
    stop("column specified as clones.to.plot was not found in GEX.output@metadata")
  }
  
  #make sure that the frequency column is numeric
  GEX.output@meta.data$clonotype_frequency <- as.numeric(as.character(GEX.output@meta.data$clonotype_frequency))
  
  #make column sample + clonotype id
  GEX.output@meta.data$s_cl <- paste0(GEX.output@meta.data$sample_id, GEX.output@meta.data$clonotype_id_10x)
  
  #get subset unique that column + frequency
  cl_unique <- GEX.output@meta.data[which(duplicated(GEX.output@meta.data$s_cl) == F),c("s_cl", "sample_id","group_id", "clonotype_id_10x", "clonotype_frequency")]
  if(clones.to.plot != "none"){ #if clones to plot is specified bind that column to cl_unique as well
    cl_unique <- cbind(cl_unique, GEX.output@meta.data[which(duplicated(GEX.output@meta.data$s_cl) == F),c(clones.to.plot)])
    names(cl_unique)[ncol(cl_unique)] <- "clonotype_to_plot"
  }
  print(head(cl_unique))
  #order by frequency
  ...
  if(by.sample == F & by.group == F){
    ...
    GEX.output@meta.data$cl_to_plot <- "Not selected"
    ...
    for(i in 1:nrow(cl_unique)){
      ...
      GEX.output@meta.data$cl_to_plot[which(GEX.output@meta.data$s_cl == cl_unique$s_cl[i])] <- paste0(i, " / ",cl_unique$sample_id[i], " / ", cl_unique$clonotype_id_10x[i], " / ", cl_unique$clonotype_frequency[i])
    ...
    #order for plotting  
    GEX.output@meta.data$cl_to_plot <- ordered(as.factor(GEX.output@meta.data$cl_to_plot), levels = c(track_cl_to_plot, "Not selected"))
    ...
    if(missing(clone.colors)) clone.colors <- rainbow(length(unique(GEX.output@meta.data$cl_to_plot))-1)
    if(length(clone.colors) != length(unique(GEX.output@meta.data$cl_to_plot))-1){stop(paste0("Nr of supplied colors ", length(clone.colors)  ," does not match number of clones to plot ", length(unique(GEX.output@meta.data$cl_to_plot))-1))}
    
    #dimplot with cols + grey30 to color in the non selected clones
    out.plot <- Seurat::DimPlot(GEX.output,reduction = reduction, group.by = "cl_to_plot", cols = c(clone.colors,"grey80"), shuffle = T, pt.size = pt.size) +labs(col = "Rank / Sample id / Clonotype / Frequency", title = "")
    #check whether plot and legend should be split
    if(split.plot.and.legend == F){
      return(out.plot)
    } else {
      out.leg <- get_legend(out.plot)
  ...
  if(by.sample == T){
    ...
    if(clones.to.plot == "none"){
      ...
      cl_unique <- bind_rows(cl_unique_all)
    ...
    for(i in 1:nrow(cl_unique)){
      #paste together clonal rank and frequency
      GEX.output@meta.data$cl_to_plot[which(GEX.output@meta.data$s_cl == cl_unique$s_cl[i])] <- paste0(r, " / ",cl_unique$sample_id[i], " / ", cl_unique$clonotype_id_10x[i], " / ", cl_unique$clonotype_frequency[i])
    ...
    if(clones.to.plot == "none"){
      if(missing(clone.colors)) clone.colors <- rep(rainbow(n.clones), length(unique(GEX.output@meta.data$sample_id)))
      if(length(clone.colors) == n.clones) clone.colors <- rep(clone.colors, length(unique(GEX.output@meta.data$sample_id)))
      if(length(clone.colors) != n.clones * length(unique(GEX.output@meta.data$sample_id))){stop("Nr of supplied colors does not match number of clones to plot. Provide either a vector of lenght n.clones or a of length Nr of samples/groups * n.clones")}
    } else{
    ...
    if(length(unique(GEX.output$cl_to_plot)) < length(clone.colors)){
      clone.colors <- clone.colors[1:(length(unique(GEX.output$cl_to_plot))-1)]
    }
    
    #dimplot with cols + grey30 to color in the non selected clones
    out.plot <- Seurat::DimPlot(GEX.output,reduction = reduction, group.by = "cl_to_plot", cols = c(clone.colors,"grey80"), shuffle = F, pt.size = pt.size) +labs(col = "Rank / Sample id / Clonotype / Frequency", title = "") + facet_wrap(~GEX.output@meta.data$sample_id, ncol = ncol.facet) 
  ...
  if(by.group == T){
    ...
    for(i in 1:nrow(cl_unique)){
      #paste together clonal rank and frequency
      GEX.output@meta.data$cl_to_plot[which(GEX.output@meta.data$s_cl == cl_unique$s_cl[i])] <- paste0(r, " / ",cl_unique$group_id[i], " / ",cl_unique$sample_id[i], " / ", cl_unique$clonotype_id_10x[i], " / ", cl_unique$clonotype_frequency[i])
    ...
    if(clones.to.plot == "none"){
      if(missing(clone.colors)) clone.colors <- rep(rainbow(n.clones), length(unique(GEX.output@meta.data$group_id)))
      if(length(clone.colors) == n.clones) clone.colors <- rep(clone.colors, length(unique(GEX.output@meta.data$group_id)))
      if(length(clone.colors) != n.clones * length(unique(GEX.output@meta.data$group_id))){stop("Nr of supplied colors does not match number of clones to plot. Provide either a vector of lenght n.clones or a of length Nr of samples/groups * n.clones")}
    } else{
      if(missing(clone.colors)) clone.colors <- rainbow(length(unique(GEX.output@meta.data$cl_to_plot))-1)
      if(length(clone.colors) != length(unique(GEX.output@meta.data$cl_to_plot))-1){stop(paste0("Nr of supplied colors ", length(clone.colors)  ," does not match number of clones to plot ", length(unique(GEX.output@meta.data$cl_to_plot))-1))}
    }
    ...
    #dimplot with cols + grey30 to color in the non selected clones
    out.plot <- Seurat::DimPlot(GEX.output,reduction = reduction, group.by = "cl_to_plot", cols = c(clone.colors,"grey80"), shuffle = F, pt.size = pt.size) +labs(col = "Rank / Group id / Sample id / Clonotype / Frequency", title = "") + facet_wrap(~GEX.output@meta.data$group_id, ncol = ncol.facet) 
    #! Bug in the current version of seurat: shuffle = T does not work with split_by or + facet_wrap(). The bug is reported and should be fixed soon (13.4.21) https://github.com/satijalab/seurat/issues/4300
