...
covid_gex <- automate_GEX(GEX.outs.directory.list = directory_to_covid_patients_integrated[1:1],integration.method = "scale.data",mito.filter = 20,cluster.resolution = 0.5,VDJ.gene.filter = T)
...
DimPlot(covid_gex[[1]],reduction = "umap")

## -----------------------------------------------------------------------------
GEX_cluster_membership(automate_GEX.output = covid_gex[[1]])
...
## -----------------------------------------------------------------------------
covid_gex_phenotype <- GEX_phenotype(covid_gex[[1]], default = T)

covid_gex[[1]] <- GEX_phenotype(covid_gex[[1]], default = F,
...
covid_heatmap_clusters <- GEX_cluster_genes_heatmap1(automate_GEX.output = covid_gex[[1]],
