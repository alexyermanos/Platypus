...
```{r}
gsea_covid <- GEX_GSEA(GEX.cluster.genes.output = gene_expression_cluster[[1]], MT.Rb.filter = T, path_to_pathways = "~/Downloads/c7.all.v7.2.symbols.gmt")
```
...
```{r, fig.show='hold'}
DE_genes_per_sample <- GEX_DEgenes_persample(automate.GEX=covid_gex[[1]],min.pct = .25,sample1 = "1",sample2 = "2",return.plot = TRUE,up.genes = 10,down.genes = 10,logFC = F)
...
```{r, fig.show='hold'}
covid_single_clone <- VDJ_per_clone(clonotype.list = covid_vdj_repertoire_bcells,VDJ.out.directory =VDJ.out.directory.list)
...
covid_single_clone_tcells <- VDJ_per_cell(clonotype.list = covid_vdj_repertoire_tcells,VDJ.out.directory =VDJ.out.directory.list_TCR)
...
# covid_single_clone was the object from VDJ_per_clone

#print(covid_single_clone[[1]][[1]]$trimmed_HC_sequence[1])  # trimmmed sequence 
#print(covid_single_clone[[1]][[1]]$trimmed_HC_germline[1])  # trimmmed germline sequence 

#stringdist::stringdist(covid_single_clone[[1]][[1]]$sequence_HC[1], VDJ.per.cell[[1]][[1]]$trimmed_ref_HC[1]) 
```
...
covid_isotypes <- VDJ_isotypes_per_clone3(VDJ_clonotype_output = covid_vdj_repertoire_bcells, VDJ_per_clone_output = covid_single_cell, clones = 30)

covid_isotypes_aa_clonotype <- VDJ_isotypes_per_clone3(VDJ_clonotype_output = covid_vdj_germline_clonotype, VDJ_per_clone_output = covid_single_cell, clones = 30)

covid_isotypes_germline_clonotype <- VDJ_isotypes_per_clone3(VDJ_clonotype_output = covid_vdj_aminoacid_clonotype, VDJ_per_clone_output = covid_single_cell, clones = 30)
...
covid_Vgene_usage <- VDJ_Vgene_usage(VDJ.clonotype.output = covid_vdj_repertoire_bcells)
library(pheatmap)

pheatmap(covid_Vgene_usage[[1]],show_rownames = F,show_colnames = F)
...
```{r}
covid_Vgene_usage_barplot <- VDJ_Vgene_usage_barplot(covid_vdj_repertoire_bcells, HC.gene.number = 10, LC.Vgene = T, LC.gene.number = 10)
print(covid_Vgene_usage_barplot[[1]])


example.vdj.vgene_usage <- VDJ_Vgene_usage_stacked_barplot(clonotype.list = covid_vdj_repertoire_bcells, LC.Vgene = F,HC.gene.number = 10, Fraction.HC = 1)
...
Platypus::VJ_alpha_beta_Vgene_circos(top10p1, c.threshold = 1,label.threshold=0,cell.level = T,V.or.J = "V")
...
```{r}
covid_CDR3_logoplot <- VDJ_logoplot(VDJ.object = covid_vdj_repertoire_bcells, length_cdr3 = 25)
```
...
covid_top10_umap <- GEX_visualize_clones(GEX.list=covid_gex,
...
covid_gex_integrate <- GEX_clonotype(GEX.object=covid_gex[[1]],VDJ.per.clone = covid_single_clone_tcells)

covid_gex_integrate_bcells <- GEX_clonotype(GEX.object=covid_gex[[1]],VDJ.per.clone = covid_single_clone)


GEX_phenotype_per_clone_plot <- GEX_phenotype_per_clone(seurat.object = covid_gex_integrate, clonotype.ids= c(1,2,3,4,5))

print(GEX_phenotype_per_clone_plot)

covid_tcell_gene_heatmap <- GEX_heatmap(covid_gex_integrate,b.or.t = "t",clone.rank.threshold = 20,sample.index = 1)


covid_bcell_gene_heatmap <- GEX_heatmap(covid_gex_integrate_bcells,b.or.t = "b",clone.rank.threshold = 20,sample.index = 1)
